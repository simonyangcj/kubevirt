...
`,
		...
		Context("virtiofs container qos", func() {
			...
			DescribeTable("should container in QOSGuaranteed group ", func(req, lim kubev1.ResourceList, dedicatedCpu, quaranteedQos bool) {
				kvConfig := &v1.KubeVirtConfiguration{
					SupportContainerResources: []v1.SupportContainerResources{
						{
							Type: v1.VirtioFS,
				...
				kvConfig.SupportContainerResources[0].Resources.Limits = lim
				clusterConfig, _, _ := testutils.NewFakeClusterConfigUsingKVConfig(kvConfig)
				qosGroupName := kubev1.PodQOSGuaranteed
				if !quaranteedQos {
					qosGroupName = kubev1.PodQOSBurstable
				}
				vmi.Spec.Domain.CPU.DedicatedCPUPlacement = dedicatedCpu
				vmi.Status.QOSClass = &qosGroupName

				res := generateVirtioFSContainers(vmi, "fakeimage", clusterConfig)
				if dedicatedCpu && quaranteedQos {
